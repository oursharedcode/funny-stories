...
- **Home screen:** two modes, selected on load by the presence of a `?room=<CODE>` URL parameter. **Create mode** (no `?room=`) shows the wordmark, the language selector (§9), a nickname field, a single "Create room" button, and a small host-stats block (open rooms; AI images today / cap — §26) — this is the host's screen. **Join mode** (`?room=<CODE>` present) shows the wordmark, a read-only "joining room `<CODE>`" line, a nickname field, and a single "Join room" button; it does **not** show the room-language selector (a joining player inherits the room's content language) or any editable room-code field. It **does** show a small joiner-only **UI-language toggle** — a single button that cycles the interface language to the next registry entry on each tap (the joiner stops when the interface reads the way they want). This toggle changes only the displayed UI language; it never changes the room's content language. A joiner's explicit pick is preserved across the join (it is not overwritten by the room's content language). There is no manual room-code entry box on either mode — the only ways to join a room are the invite link and the QR code, both of which carry `?room=<CODE>`. If a join attempt returns `ROOM_NOT_FOUND` (a stale or recycled link), Join mode shows a clear "this room no longer exists" message and a "Create a new room" button that returns to Create mode.
...
- **Round screen:** question text large and friendly, textarea below, Submit button pinned to bottom of upper half, timer bar at very top. **Joiners** (not the host) also see a small constant **story-language indicator** — flag + native name of the room's content language — between the question and the textarea, a quiet reminder of which language to write the answer in. The host already chose that language, so the indicator is hidden for the host. When a joiner's **UI language differs** from the room's content language, the question is shown **dual-display**: the primary heading stays in the room's content language (the language the answer must be written in), with a small secondary **gloss** of the same question in the joiner's UI language directly beneath it — aiding comprehension without luring a wrong-language answer. The gloss is suppressed when the two languages match (so the host, and any joiner who kept the room language as their UI, never see a redundant duplicate).
...
- `home.*` — title, subtitle, nickname placeholder, create / join buttons, joining-room label, stale-link ("room no longer exists") message, "create a new room" button, open-room counter label, images-today line, language toggle labels, `home.uiLanguage` (aria-label / tooltip for the joiner's UI-language cycle button; see §8). (No room-code placeholder — Join mode has no editable code field; see §8.)
...
- `round.*` — submit button, textarea placeholder, character counter format (`{count} / 70`), `round.storyLanguage` (label for the joiner-only story-language indicator; see §8).
- `questions` — top-level array of the 7 round prompts, keyed by round index, used only for the joiner's dual-display question gloss (§8/§9). Mirrors the authoritative `questions` array in `server/src/i18n/<code>.json`.
...
On `HomeScreen`, the language selector sets the room language and is shown **only in Create mode** (§8) — it is the host's choice. For the host the room's content language and the UI language are one and the same: the single selector sets both at once. It is a **flagged list**: one row per language showing the language's flag emoji and its native name ("English", "Русский"), rendered from the `client/src/languages.ts` registry (`{ code, name, flag }` per language). That registry is the single source for the language list — the selector and the `?lang=` URL check both derive from it.

Join mode does not show that room-language selector: a joining player inherits the room's content language automatically (the `room:join` ack carries the room `language`). Optionally, the invite link may carry the room language as `?room=<CODE>&lang=<code>` so the join screen renders in the room's language before the join completes; the flow is correct without it. **For joiners only, the two notions of "language" are decoupled:** the room's content language is fixed (shown as a constant indicator on the round screen, §8), but the **UI language** is independently adjustable via a small cycle button on the join screen (`cycleUiLanguage` in `HomeScreen.tsx`) that advances through the same registry one step per tap and calls only `i18n.changeLanguage`, never touching the room's content language. A joiner's explicit UI-language pick is preserved through join — the `room:join` ack's content language only sets the UI default when the joiner has not made a pick.

Because a joiner can read the interface in one language while the answer must be written in another, the round screen renders questions **dual-display** (§8): the primary heading is the room-language question the server sends in `round:start`, and a secondary gloss shows the same question in the joiner's UI language (suppressed when the two match). The gloss text is rendered from a `questions` array bundled in the client `i18n/<code>.json` files, keyed by round index — a client-side mirror of the authoritative per-language `questions` set in `server/src/i18n/<code>.json` (the only copy ever sent over the wire). The two must be kept in sync when question wording changes; the answer language, and thus the primary heading, always remains the room's content language.
...
- **Layout.** Below the existing prose/nickname block, separated by a vertical pad: notice (italic, muted, wrapped); QR 96 × 96 px left + label/URL block right, vertically centred against the QR's height. Rooms in every language use the same QR (the GitHub repo is multilingual) and the active locale's translation of both the notice and the source label.
